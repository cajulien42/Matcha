...
class Validator {
  ...
  constructor(format, data) {
    //{required : ['username', 'password' , 'email', 'birthyear'], optional: ['optional']} 
    this.required = _.pick(format, 'required');
    this.optional = _.pick(format, 'optional');
    this.data = data;
  ...
  validate() {
    return new Promise ((resolve) => {
      ...
      if (this.optional['optional']) schema.optional = Joi.string().alphanum().min(3).max(30)
